...
from app.schemas import DropRequest, CategoryDropRequest, RarityDropRequest, TagSearchRequest, TagDropRequest, SimulationRequest, LuckDropRequest, LuckSimulateRequest
...
def simulate_with_luck(req: LuckSimulateRequest):
    # Safety limits (important for RapidAPI)
    if req.simulations > 100_000:
        raise HTTPException(400, "Simulation limit exceeded")
    ...
    # 2. Apply luck BEFORE simulation
    adujusted_items = apply_luck(items, luck)
    
    # 3. Run simulation
    drops = simulate_drops(adujusted_items, rng, req.simulations)
    
    # 4. Analyze results (same as /simulations)
    rarity_counts = {}
    tag_counts = {}
    item_counts = {}
    ...
    # 5. Generate warnings (reuse logic)
    warnings = []
